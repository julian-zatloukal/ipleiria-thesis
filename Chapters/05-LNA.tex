...
\begin{itemize}
\item \textbf{Corriente de colector}: $20mA$ (a temperatura ambiente)
\item \textbf{Figura de ruido}: $NF<1.4 dB$
\item \textbf{Ganancia de potencia}: aproximadamente $12 dB$
\item \textbf{Return Loss de entrada}: menor a $−8 dB$
\item \textbf{Return Loss de salida}: menor a $−10 dB$
\end{itemize}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Figures/LNA/AN11432_cir.png}
\captionsetup{justification=centering}
\caption{Circuito propuesto por NXP en su \\ nota de aplicación AN11432}
\end{figure}
...
Para el análisis del punto de polarización se utilizó el software ADS, fijando una corriente de colector de 20 mA y obteniendo una tensión colector–emisor cercana a 3V. Se eligió esta punto de polarización debido a que fue el seleccionado en la nota de aplicacion AN11432 en la cual se busco un muy bajo ruido con una buena ganancia. El análisis DC se realizó mediante un template específico de ADS, que permite evaluar las características estáticas del transistor en función de la corriente y la tensión aplicadas. Tambien se grafico la $NF_{min}$ la cual seria el piso de ruido que nos daria el transistor configurado con una $V_{CE}=3V$ y una $I_C=20mA$. Esta figura de ruido limitara el piso de ruido que puede tener el LNA.



\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{Figures/LNA/BFU550W_nf_min.png}
\captionsetup{justification=centering}
...
% nf[2]
\addplot[blue, thick]
table[x index=3, y index=1] {Datasets/5_LNA/noise_alone.dat};
\addlegendentry{nf[2]}

% NFmin
\addplot[green!60!black, thick]
table[x index=3, y index=2] {Datasets/5_LNA/noise_alone.dat};
...
\textbf{En 0.9 GHz} \\[2pt]
nf[2] = 1.48 \\
$NF_\mathrm{min}$ = 0.933
};
...
Para el gráfico de $NF_{min}$ se utilizo otro template de ADS. Este template nos permitió observar el piso de ruido que tendríamos debido a la utilización del BFU550W evaluado con una tensión de colector emisor de 3V y una corriente de colector de 20mA.



\subsection{Ganancia:}
La ganancia del amplificador se evaluó a partir del parámetro de dispersión $S_{21}$, que relaciona la onda incidente en la entrada con la onda transmitida hacia la carga. Según la información del fabricante, el transistor BFU550W puede alcanzar una ganancia máxima de aproximadamente 18 dB para una corriente de colector de 20 mA.

En las simulaciones realizadas se obtuvo una ganancia máxima cercana a 15,5 dB alrededor de 430 MHz, mientras que a 900 MHz la ganancia se reduce a aproximadamente 12,6 dB. Este comportamiento es consistente con la respuesta en frecuencia del dispositivo y con la adaptación implementada en el circuito.
...
% nf[2]
\addplot[blue, thick]
table[x index=0, y index=1] {Datasets/5_LNA/parametro_S21.dat};

\end{axis}
\end{tikzpicture}
\caption{Ganancia del circuito propuesto en la nota de aplicacion AN11432}
\label{fig:nf_vs_freq}
\end{figure}
...
Inicialmente se analizó la figura de ruido mínima  considerando únicamente el transistor, en un sistema ideal. En estas simulaciones se observan que la figura de ruido del BFU550W presenta un valor menor a 1dB en todo el rango a utilizar. Sin embargo, al incorporar nuestro
una carga de 50 Ω resulta necesaria una red de adaptación, lo que inevitablemente introduce un incremento en la figura de ruido total.
...
% nf[2]
\addplot[blue, thick]
table[x index=3, y index=1] {Datasets/5_LNA/ruido_AN11432.dat};
\addlegendentry{nf[2]}

% NFmin
\addplot[green!60!black, thick, mark=triangle*]
table[x index=3, y index=2] {Datasets/5_LNA/ruido_AN11432.dat};
\addlegendentry{$NF_\mathrm{min}$}
...
% nf[2]
\addplot[blue, thick]
table[x index=2, y index=0] {Datasets/5_LNA/mu_AN11432.dat};
\addlegendentry{Mu1}

% NFmin
\addplot[green!60!black, thick]
table[x index=2, y index=1] {Datasets/5_LNA/mu_AN11432.dat};
\addlegendentry{MuPrime1}

\end{axis}
\end{tikzpicture}
\caption{Estabilidad obtenida del circuito propuesto en AN11432}
\label{fig:nf_vs_freq}
\end{figure}
...
\subsection{Punto de compresión de 1 dB (P1dB):}
El punto de compresión de 1 dB es un parámetro fundamental para caracterizar los límites de operación lineal del LNA. Se define como el nivel de potencia de entrada para el cual la ganancia del amplificador se reduce en 1 dB respecto de la ganancia obtenida en régimen lineal.
...
\begin{axis}[
    width=0.95\textwidth,
    height=7cm,
    xlabel={Potencia de entrada [mdB]},
    ylabel={Potencia en la salida [mdB]},
    grid=both,
    grid style={line width=.1pt, draw=gray!30},
    major grid style={line width=.2pt, draw=gray!50},
    tick label style={font=\footnotesize},
...
\addplot[blue, thick]
table[x index=0, y index=1] {Datasets/5_LNA/power_AN11432.dat};


\end{axis}
\end{tikzpicture}
\caption{Punto de compresión del circuito propuesto en AN11432}
\label{fig:nf_vs_freq}
\end{figure}
...
A bajas potencias de entrada, la relación entre la potencia de entrada y la potencia de salida es aproximadamente lineal. Sin embargo, al incrementar la potencia, el dispositivo comienza a operar en una región no lineal, produciéndose una saturación parcial. En las simulaciones se observó que este fenómeno comienza a manifestarse para potencias de entrada del orden de −3 dBm.
...
\addplot[blue, thick]
table[x index=0, y index=4] {Datasets/5_LNA/power_AN11432_nor.dat};
...
\addplot[green, thick]
table[x index=0, y index=2] {Datasets/5_LNA/power_AN11432.dat};
\addlegendentry{$dB(Armonico_2)$}

\addplot[red, thick]
table[x index=0, y index=3] {Datasets/5_LNA/power_AN11432.dat};
\addlegendentry{$dB(Armonico_3)$}

\end{axis}
\end{tikzpicture}
\caption{Evolucion de la potencia del segundo y tercer\\ armonico en el circuito propuesto en AN11432a}
\label{fig:nf_vs_freq}
\end{figure}
...
Como se pudo observar en este análisis, se obtuvo un bajo nivel de ruido junto con una ganancia adecuada, teniendo una figura de ruido menor a 1.5dB y una ganancia de 12.6dB en nuestra frecuencia de trabajo. Sin embargo, los resultados obtenidos difieren levemente de los presentados en la nota de aplicación, a pesar de que en ambos casos se utilizaron los mismos componentes.

En particular, nuestro análisis arrojó una figura de ruido aproximadamente 1 dB mayor y una ganancia inferior, la cual no se encuentra centrada en el punto de interés. No obstante, la ganancia obtenida en nuestras simulaciones sigue siendo buena en nuestra frecuencia de trabajo.
