...
Para el presente diseño del radar FMCW con frecuencia de operación centrada en \qty{1}{\GHz}, el VCO debe cumplir con los siguientes requerimientos:

\begin{itemize}
    \item \textbf{Rango de frecuencia:} El VCO debe ser capaz de oscilar en un rango que incluya la frecuencia central de \qty{1}{\GHz}. Para este diseño se adoptó un ancho de banda de barrido de $BW = \qty{50}{\MHz}$, lo que resulta en una resolución en distancia de $\Delta R = \frac{c}{2 \cdot BW} = \frac{\qty{3e8}{\m\per\s}}{2 \times \qty{50e6}{\Hz}} = \qty{3}{\m}$.
...
\midrule
CJO & Capacitancia de unión con polarización cero & \num{4,2} & \unit{\pF} \\
VJ & Potencial de unión & \num{1,7} & \unit{\V} \\
M & Coeficiente de gradación & \num{0,9} & -- \\
CP & Capacitancia del encapsulado (paralelo) & \num{0} & \unit{\pF} \\
RS & Resistencia en serie & \num{1,5} & \unit{\ohm} \\
LS & Inductancia en serie & \num{1,7} & \unit{\nH} \\
\bottomrule
\end{tabular}
\end{table}

Para encontrar los valores óptimos de los componentes se configuraron los controladores de simulación tal como se muestra en la Figura \ref{fig:vco_optimization_controllers}, utilizando un análisis de \textit{Harmonic Balance}, un optimizador y dos objetivos a cumplir. El análisis de \textit{Harmonic Balance} está configurado en modo oscilador (\texttt{OscMode=yes}), con una frecuencia fundamental inicial de búsqueda de \qty{1}{\GHz} y un barrido de la variable $V_{\text{tune}}$ de \qty{0}{\V} a \qty{3}{\V}. Los capacitores $C_{\text{couple}}$ y $C_t$ se definen como variables optimizables. Se establecieron dos objetivos de optimización: la frecuencia mínima ($f_{\text{min}}$) al aplicar \qty{0}{\V} debe estar en el rango de \qty{975}{\MHz} a \qty{975,5}{\MHz}, y la frecuencia máxima ($f_{\text{max}}$) al aplicar \qty{3}{\V} debe ser mayor a \qty{1,025}{\GHz}. El optimizador utiliza un algoritmo de tipo aleatorio (\textit{Random}) con un máximo de 100 iteraciones.
...
Los resultados obtenidos fueron $C_{\text{couple}} = \qty{2,82587}{\pF}$ y $C_t = \qty{0,565855}{\pF}$, adoptándose los valores comerciales de \qty{2,9}{\pF} y \qty{0,55}{\pF} para los análisis posteriores. En la Figura \ref{fig:vco_fosc_vtune} se puede observar el gráfico de $f_{\text{osc}}$ en función de la tensión de entrada $V_{\text{tune}}$.
...
De la curva se comprueba que para generar la señal chirp de \qty{50}{\MHz} (de \qty{975}{\MHz} a \qty{1025}{\MHz}) a la salida del VCO, se necesita una señal de diente de sierra a la entrada que varíe aproximadamente de \qty{0,035}{\V} a \qty{1,105}{\V}. Una linealización de la curva en la región activa (\qty{0,6}{\V} a \qty{2,5}{\V}) propone la siguiente ecuación de transferencia:

\begin{equation}
f(V_{\text{tune}}) = \num{67,89} \cdot V_{\text{tune}} + \num{948,89} \quad [\unit{\MHz}]
...
La primera condición de arranque de oscilación requiere que la fase de $S_{11}$ sea de \qty{0}{\degree} a la frecuencia de oscilación. De los resultados se obtiene que a \qty{1,132}{\GHz} la fase es de aproximadamente \qty{0,098}{\degree}, cumpliendo esta condición. La segunda condición establece que la magnitud de $S_{11}$ debe ser mayor que 1; en este caso, la ganancia del lazo es de \num{5,866} (en veces), verificando así la condición de arranque.
...
En la Figura \ref{fig:vco_spectrum} se puede ver el resultado obtenido. Se obtuvo como componente fundamental \qty{-0,635}{\dBm} en la frecuencia central de \qty{1}{\GHz}, lo cual es un nivel de potencia aceptable como entrada del amplificador de potencia. La segunda y tercera armónica tienen niveles de \qty{-13,49}{\dBm} y \qty{-26,29}{\dBm} respectivamente. La relación de potencia fundamental a segunda armónica es de \qty{12,85}{\dBc}, y a tercera armónica es de \qty{25,65}{\dBc}, valores aceptables para esta aplicación.
...
Se analiza el valor complejo de impedancia de entrada del núcleo (\textit{core}) del oscilador. En la Figura \ref{fig:vco_impedance_analysis} se puede observar el setup utilizado para simular este parámetro, donde se conecta un puerto de prueba al nodo superior del circuito y se realiza un barrido de parámetros S desde \qty{0,5}{\GHz} hasta \qty{3}{\GHz}.
...
\node[anchor=south west, font=\scriptsize, fill=white, inner sep=2pt] at (axis cs:1.05,-85.01) {\qty{-69,0}{\ohm} @ \qty{1,005}{\GHz}};
...
A la frecuencia de interés (aproximadamente \qty{1}{\GHz}), la parte real de la impedancia es de \qty{-69,0}{\ohm}. Este valor de resistencia negativa es robusto y significa que el oscilador podrá arrancar incluso si el tanque LC tiene pérdidas resistivas de hasta \qty{69}{\ohm}. En diseño práctico se busca que $|R_{\text{neg}}|$ sea 2 o 3 veces mayor que la resistencia de carga para asegurar un arranque rápido y confiable.
...
\node[anchor=south west, font=\scriptsize, fill=white, inner sep=2pt] at (axis cs:1.05,-35.40) {\qty{-23,4}{\ohm} @ \qty{1,005}{\GHz}};
...
La parte imaginaria a \qty{1,005}{\GHz} es de \qty{-23,4}{\ohm}, lo que indica que el núcleo del oscilador tiene un comportamiento capacitivo a esta frecuencia. Para que el circuito resuene, el tanque resonante externo debe presentar una reactancia inductiva que cancele esta capacitancia, cumpliendo la condición de resonancia $\text{Im}(Z_{\text{total}}) = 0$.

En resumen, a la frecuencia de operación de \qty{1}{\GHz}, el núcleo presenta una impedancia de entrada de $Z_{\text{in}} = (-69{,}0 - j23{,}4)\,\unit{\ohm}$, confirmando que el circuito está correctamente polarizado y genera suficiente resistencia negativa para sostener las oscilaciones.

\subsection{Período Transitorio y Estado Estacionario en el Dominio del Tiempo}

Se analiza el transitorio y el estado del sistema luego del mismo, en el dominio del tiempo. En la Figura \ref{fig:vco_transient_analysis} se presenta el esquemático de la simulación, donde se configuran dos análisis transitorios: uno de \qty{0}{\ns} a \qty{50}{\ns} para observar el arranque, y otro de \qty{990}{\ns} a \qty{1000}{\ns} para verificar el estado estacionario.
...
En la Figura \ref{fig:vco_startup} se observa el período transitorio del oscilador. Durante los primeros \qty{20}{\ns} aproximadamente, el oscilador amplifica únicamente ruido térmico con amplitud infinitesimal. Entre \qty{20}{\ns} y \qty{30}{\ns} se produce un crecimiento exponencial de la oscilación, confirmando que la condición $|R_{\text{neg}}| > R_{\text{pérdidas}}$ se cumple correctamente. A partir de los \qty{30}{\ns}, la amplitud comienza a saturarse debido a las no linealidades del transistor. Se puede observar que el sistema alcanza el estado estacionario luego de aproximadamente \qty{40}{\ns} a \qty{50}{\ns}.
...
En la Figura \ref{fig:vco_steady_state} se observa el estado estacionario del sistema. Si bien el período coincide con el de una señal de \qty{1}{\GHz} (aproximadamente 10 ciclos en \qty{10}{\ns}), se nota una deformación en la señal: las crestas superiores son más redondeadas mientras que los picos inferiores son más agudos. Este es un comportamiento esperado en este tipo de osciladores de gran señal (\textit{Large Signal}). La tensión de salida varía aproximadamente de \qty{-410}{\mV} a \qty{+280}{\mV}, resultando en una amplitud pico a pico de aproximadamente \qty{690}{\mV}.
...
Se diseñó y simuló exitosamente un Oscilador Controlado por Tensión basado en la topología Clapp modificada en configuración colector común, operando a una frecuencia central de \qty{1}{\GHz}. El VCO cumple con los requerimientos establecidos: presenta un rango de sintonía de \qty{50}{\MHz} controlado mediante una tensión de entrada que varía de \qty{0}{\V} a \qty{1,1}{\V}, entrega una potencia de salida de \qty{-0,6}{\dBm} en la componente fundamental, y demuestra un arranque robusto en aproximadamente \qty{40}{\ns}. Los análisis de estabilidad confirman que el circuito genera suficiente resistencia negativa para sostener las oscilaciones, y las simulaciones en el dominio del tiempo verifican el correcto funcionamiento del oscilador en estado estacionario.
