...
% Línea de referencia -10 dB (dibujada primero para quedar detrás)
\addplot[black, dashed, line width=0.8pt, forget plot] coordinates {(0.5,-10) (1.5,-10)};
\node[anchor=south west, font=\scriptsize, fill=white, inner sep=1pt] at (axis cs:0.52,-10) {$-10$ dB};

% Línea de referencia -20 dB
\addplot[black, dashed, line width=0.8pt, forget plot] coordinates {(0.5,-20) (1.5,-20)};
...
% Línea de referencia 12 dB (dibujada primero para quedar detrás)
\addplot[black, dashed, line width=0.8pt, forget plot] coordinates {(0.5,12) (1.5,12)};
\node[anchor=south west, font=\scriptsize, fill=white, inner sep=1pt] at (axis cs:0.65,12) {$12$ dB};
...
% Línea de referencia del óptimo (dibujada primero para quedar detrás)
\addplot[black, dashed, line width=1pt, forget plot] coordinates {(0.855,{-8}) (0.855,{14})};
\node[anchor=south, font=\scriptsize, fill=white, inner sep=1pt, rotate=90] at (axis cs:0.856,6) {Óptimo: 0.855 V};
...
En este punto, el diseño del amplificador está prácticamente completo en el simulador, habiendo logrado una adaptación de impedancias optimizada para \qty{1}{\GHz} con $S_{11} < \qty{-25}{\dB}$, un punto de operación DC optimizado a $\approx \qty{0,855}{\V}$ para obtener la máxima ganancia disponible, y una ganancia de aproximadamente \qty{11,6}{\dB} a la frecuencia central, cercano al objetivo de \qty{12}{\dB}.
...
% Línea de referencia K=1 (dibujada primero para quedar detrás)
\addplot[black, dashed, line width=0.8pt, forget plot] coordinates {(0,1) (15,1)};
...
\subsection{Análisis en el Dominio Temporal: Formas de Onda de Colector, Fuente y Carga}

Se analizan las formas de onda del amplificador en dos ventanas temporales: el transitorio (\qty{0}{\ns} a \qty{40}{\ns}) en la Figura \ref{fig:pa_transient_waveforms}, y el estado estacionario (\qty{1}{\us} a \qty{1,004}{\us}, equivalente a 4 períodos a \qty{1}{\GHz}) en la Figura \ref{fig:pa_steady_state_waveforms}.
...
% Vc - Tensión de colector
\nextgroupplot[
    ylabel={$V_C$ [V]},
    xmin=0, xmax=40,
    ymin=0, ymax=12,
    title style={font=\small\bfseries},
    title={Tensión de Colector},
]
\addplot[red, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_vc.dat};
...
\nextgroupplot[
    ...
    ymin=-30, ymax=150,
    title style={font=\small\bfseries},
    title={Corriente de Colector},
]
\addplot[blue, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_i_c_i.dat};

% Vcc - Tensión de fuente
\nextgroupplot[
    ylabel={$V_{CC}$ [V]},
    xmin=0, xmax=40,
    ymin=7.9, ymax=8.1,
    title style={font=\small\bfseries},
    title={Tensión de Fuente},
]
\addplot[red, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_vcc.dat};

% I_dc - Corriente de fuente DC
\nextgroupplot[
    ylabel={$I_{DC}$ [mA]},
    xmin=0, xmax=40,
    ymin=-30, ymax=150,
    title style={font=\small\bfseries},
    title={Corriente de Fuente DC},
]
\addplot[blue, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_i_dc_i.dat};

% Vrl - Tensión en carga
\nextgroupplot[
    ylabel={$V_{RL}$ [V]},
    xmin=0, xmax=40,
    ymin=-1.5, ymax=1.5,
    title style={font=\small\bfseries},
    title={Tensión en Carga},
]
\addplot[red, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_vrl.dat};

% I_rl - Corriente en carga
\nextgroupplot[
    ylabel={$I_{RL}$ [mA]},
    xmin=0, xmax=40,
    ymin=-30, ymax=30,
    title style={font=\small\bfseries},
    title={Corriente en Carga},
]
\addplot[blue, thick, no markers] table[x expr=\thisrowno{0}*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran1_i_rl_i.dat};

\end{groupplot}
\end{tikzpicture}
\captionsetup{justification=centering}
\caption{Formas de onda durante el transitorio del PA (0 a 40 ns). \\ Izquierda: tensiones. Derecha: corrientes.}
\label{fig:pa_transient_waveforms}
\end{figure}
...
% Vc - Tensión de colector (estado estacionario)
\nextgroupplot[
    ylabel={$V_C$ [V]},
    xmin=0, xmax=4,
    ymin=4, ymax=12,
...
\addplot[red, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_vc.dat};

% I_c - Corriente de colector (estado estacionario)
\nextgroupplot[
    ylabel={$I_C$ [mA]},
    xmin=0, xmax=4,
    ymin=-10, ymax=70,
...
\addplot[blue, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_i_c_i.dat};

% Vcc - Tensión de fuente (estado estacionario)
\nextgroupplot[
    ylabel={$V_{CC}$ [V]},
    xmin=0, xmax=4,
    ymin=7.9, ymax=8.1,
...
\addplot[red, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_vcc.dat};

% I_dc - Corriente de fuente DC (estado estacionario)
\nextgroupplot[
    ylabel={$I_{DC}$ [mA]},
    xmin=0, xmax=4,
    ymin=-10, ymax=70,
...
\addplot[blue, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_i_dc_i.dat};

% Espacio vacío para Vrl (no disponible en tran2)
\nextgroupplot[
    ylabel={$V_{RL}$ [V]},
    xmin=0, xmax=4,
    ymin=-1.5, ymax=1.5,
...
\addplot[red, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_vrl.dat};

% I_rl - Corriente en carga (estado estacionario)
\nextgroupplot[
    ylabel={$I_{RL}$ [mA]},
    xmin=0, xmax=4,
    ymin=-30, ymax=30,
...
\addplot[blue, thick, no markers] table[x expr=(\thisrowno{0}-1e-6)*1e9, y expr=\thisrowno{1}*1000, col sep=tab] {Datasets/2_PA/transient_parsed/tran2_i_rl_i.dat};

\end{groupplot}
\end{tikzpicture}
\captionsetup{justification=centering}
\caption{Formas de onda en estado estacionario del PA (4 períodos a 1 GHz). \\ Izquierda: tensiones. Derecha: corrientes.}
\label{fig:pa_steady_state_waveforms}
\end{figure}

\section{Conclusiones}

Se ha diseñado exitosamente un amplificador de potencia en Clase AB para la banda de \qty{1}{\GHz}, partiendo de la nota de aplicación AN11501 de NXP como referencia. Mediante un proceso sistemático de optimización, se resintonizaron las redes de adaptación de entrada y salida, y se ajustó el punto de polarización a $V_{bias} = \qty{0,855}{\V}$. El diseño final presenta una ganancia de \qty{10,97}{\dB} con $S_{11} < \qty{-25}{\dB}$ a la frecuencia central, cumpliendo los requisitos de adaptación especificados. El análisis de estabilidad reveló oscilaciones parásitas que fueron mitigadas mediante la adición de una resistencia de estabilización de \qty{695}{\ohm}, garantizando $K > 1$ en todo el rango de frecuencias. La distorsión armónica total del \qty{0,27}{\percent} y la supresión del segundo armónico de \qty{-51}{\dBc} confirman la idoneidad del amplificador para la aplicación radar FMCW.
